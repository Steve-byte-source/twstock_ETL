...
def daily_report(date):
    ...
    Transaction = spark.read.parquet(f"hdfs://devenv/user/spark/twstock/raw_data/broker_transaction/{date}.parquet")
    ...
    Open_Close = spark.read.parquet(f"hdfs://devenv/user/spark/twstock/raw_data/transactions/{date}.parquet")
    ...
    daily_report = daily_report.withColumn("sID",col("sID").cast(IntegerType()))\
...
#                                  ).mode('append').save()
    daily_report.repartition(1).write.parquet(f"hdfs://devenv/user/spark/twstock/analysis/daily_report_1/{date}")